...
def callback_filter(data):
    ...
    blue_percentage=data.data
    red_percentage=data.data
    green_percentage=data.data
...
def callback(data):
    ...
    if state == 8:
        ...
        if (forward_dist>0.95) and (left_dist<1 or right_dist<1):
	    checkpoint=checkpoint+1+checkpoint1
            state=9

    if state==9:

        if checkpoint==7 and (blue_percentage>6000 or green_percentage>6000):
	    if (blue_percentage>6000):
	        state=13
	    else:
		state=14
    ...
    if state == 10:
        ...
        if right_dist > 1.1 and left_dist < 0.8:
            state=11
            
        if left_dist > 1.1 and right_dist < 0.8:
            state=12
    ...
    print("yaw: ", yaw)
    print("Blue_Filt: ",blue_percentage)
    pub.publish(vel)
