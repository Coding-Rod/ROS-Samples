...
def callback(data):
    ...
    T04n=T04.subs([(t1,data.x),(t2,data.y),(t3,data.z),(l1,0.4),(l2,0.3),(l3,0.3)]) #modify
    position.x= T04n[0,3] #modify variable name T0Xn
    position.y= T04n[1,3] #modify variable name T0Xn
    position.z= T04n[2,3] #modify variable name T0Xn
    pos.publish(position)
    print(angles)

def listener():
    rospy.init_node('for_kin_PR2', anonymous=True)   
